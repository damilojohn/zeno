...
async def authenticate_user(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail=f"failed to authenticate with error {e}",
        ) from e
...
async def reset_password(email: str, db_session: AsyncSession):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"password reset failed with error {e}"
        )
    try:
    ...
    return ResetTokenResponse(msg="Password Reset mail sent successfully!!")


async def create_new_password(new_password, token: str, db_session: AsyncSession):
    try:
        # get token hash
        ...
        if db_result:
            ...
            if curr_time > to_expire:
                ...
                raise HTTPException(
                    status_code=HTTP_400_BAD_REQUEST, detail="Reset Token expired"
                )
            else:
                ...
                try:
                    ...
                    if db_user:
                        ...
                        delete_result = await db_session.execute(
                            select(ResetTokens).filter(
                                ResetTokens.user_id == db_user.id
                            )
                        )
                        ...
                        for token_to_delete in tokens_to_delete:
                            ...
                            LOG.info(
                                "Successfully deleted all reset tokens for user..."
                            )
                            await db_session.commit()
...
async def handle_google_oauth(
    code: str, redirect_uri: str, session: AsyncSession = Depends(get_async_db_session)
):
    try:

...
async def scrape_goodreads_popular_books(pages=5):
    ...
    async with RetryClient(retry_options=retry_options) as session:
        for page in range(1, pages + 1):
            ...
            book_urls = [item.find('a', {'class': 'bookTitle'})['href'] for item in book_items]
            tasks = [asyncio.create_task(scrape_book_page(session, f"https://www.goodreads.com{url}")) for url in book_urls]
            
            page_books = await asyncio.gather(*tasks)
            books.extend(page_books)
            
            print(f"Scraped page {page}")
            await asyncio.sleep(random.uniform(1, 3))  # Be polite, add a delay between pages
